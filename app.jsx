...
const GUIDE_STEPS = [
  { icon: "login", title: "Как сюда войти",
    ...
    list: [
      ["Впишите почту и пароль", ", нажмите **«Войти»**."],
      ["Заходить заново не нужно", ", программа вас запомнит. Закрыли, открыли, вы внутри."],
      ["Телефон или компьютер", ", без разницы. Прогресс общий, он привязан к почте."],
    ] },
  ...
  { icon: "chart", title: "Отметьте, как вам сейчас",
    body: ["В конце дня есть блок **«Состояние за деньги»**."],
    list: [
  ...
  { icon: "sunrise", title: "Завтра вернитесь за новым днём",
    body: ["Дни открываются по одному, каждое утро в **8:00**."],
    list: [
      ["Завтрашний день закрыт", ", на нём замочек и точная дата, например «Откроется 27 июня»."],
  ...
  return (
    <div className="page guide-col">
      ...
      <div className="card gd-step">
        ...
        <div className="gd-body">
          <p>Это не обязательно. Но если вы проходите программу с телефона, для удобства можно поставить иконку на главный экран. Тогда «Протокол денег» открывается в один тап, как обычное приложение, а иконкой будет зелёный рубль.</p>
          <ul className="gd-list">
            <li><b>Android.</b> Нажмите кнопку ниже, телефон сам предложит установить.</li>
            <li><b>Айфон, Safari.</b> Делается вручную, четыре шага показаны на картинке ниже.</li>
          </ul>
          <div className="gd-tip"><Ico.info /><span>Открываете в другом браузере (Яндекс, Chrome)? Пункт может называться «Добавить ярлык на телефон». Смысл тот же.</span></div>
          <div className="gd-install"><InstallButton force /></div>
        </div>
        <GuideShot src="guide-img/ios-install.png" cap="Айфон, Safari: «...», затем «Поделиться», пролистать до «На экран Домой», затем «Добавить»." />
      </div>
