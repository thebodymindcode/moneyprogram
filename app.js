...
const GUIDE_STEPS = [{
  ...
  body: ["Вы заходите по своей почте и паролю. Это та самая почта, по которой вам открыли доступ. Впишите её, впишите пароль и нажмите «Войти».", "Заходить каждый раз заново не нужно. Программа вас запомнит. Закрыли страницу, открыли снова, и вы по-прежнему внутри.", "Зайти можно и с телефона, и с компьютера. Всё работает одинаково."]
}, {
  icon: "download",
  title: "Поставьте иконку на телефон",
  install: true,
  body: ["Чтобы заходить в один тап, добавьте «Протокол денег» на главный экран телефона. Появится иконка, как у обычного приложения, и программа будет открываться сразу, без браузера.", "Нажмите кнопку ниже. На Android телефон сам предложит установить. На айфоне откроется короткая подсказка, куда нажать (Поделиться, затем пункт «На экран Домой»)."]
}, {
  icon: "grid",
  ...
  list: [["Мой прогресс.", " Главная страница. Видно, сколько дней пройдено, какой день сегодня, и график вашего спокойствия."], ["Карта дней.", " Все 17 дней по порядку. Сегодня открыт, следующий виден названием, остальные под замком с датой открытия."], ["Дневник.", " Сюда сами собираются все ваши ответы и заметки за все дни. В одном месте."], ["Инструкция.", " Эта страница. Сюда можно вернуться в любой момент, если что-то забылось."]],
  shot: {
  ...
  body: ["Наверху страницы дня стоит проигрыватель. Нажмите круглую кнопку с треугольником, и аудио начнёт играть. Чтобы поставить на паузу, нажмите ту же кнопку ещё раз.", "Под полоской времени можно поменять скорость: кнопки 1,3, 1,5 и 1,8, или ползунок для точной настройки. Голос остаётся ровным, не ускоряется в писк.", "Слушать удобнее в наушниках или просто включить погромче.", "Аудио не включилось с первого раза? Не переживайте. Обновите страницу и нажмите кнопку снова. Чтобы обновить, потяните страницу пальцем сверху вниз."],
  shot: {
  ...
  body: ["Ниже текста идут задания дня. Их немного. Под каждым заданием есть поле, куда можно вписать ответ.", "Впишите ответ своими словами и нажмите кнопку «Готово». Задание отметится зелёной галочкой. Когда отмечены все задания, день засчитан.", "Ответ сохраняется сам. Отдельно сохранять ничего не надо. Захотите поправить ответ, нажмите «Изменить»."],
  shot: {
  ...
  body: ["В самом низу дня есть «Заметка дня». Это место только для вас: запишите, что почувствовали и что поняли. Без оценок и без «правильно или неправильно».", "Заметка сохраняется сама, как только вы нажмёте в другое место. Потом все заметки и ответы соберутся в разделе «Дневник»."],
  shot: {
  ...
  body: ["В конце дня есть блок «Состояние за деньги». Подвиньте ползунок туда, где вы сейчас, между тревогой и спокойствием. Это один штрих, пара секунд.", "Отметка сохраняется сама. День за днём она собирается в график на странице «Мой прогресс», и вы своими глазами видите, как становится спокойнее."]
}, {
  icon: "sunrise",
  title: "Завтра вернитесь за новым днём",
  body: ["Дни открываются по одному, каждое утро в 8:00. Поэтому завтрашний день пока закрыт. На нём виден замочек и точная дата, когда он откроется, например «Откроется 27 июня».", "Так у вас каждый день один спокойный шаг, без спешки и без соблазна забежать вперёд.", "Завтра просто зайдите снова и откройте новый день. Пропустили день, ничего страшного. Он не пропадёт, спокойно вернётесь к нему позже."],
  shot: {
...
const GUIDE_FAQ = [{
  ...
  a: "Дни открываются по одному, каждое утро в 8:00. Если день закрыт, на нём замочек и дата открытия. Дождитесь этой даты и обновите страницу."
}, {
  ...
  a: "Это «Состояние за деньги» в конце дня. Отмечайте, где вы между тревогой и спокойствием. День за днём собирается график вашего спокойствия на «Моём прогрессе»."
}, {
  q: "Аудио не играет",
...
function Guide() {
  ...
  return React.createElement("div", {
    ...
    return React.createElement("div", {
      ...
      key: j
    }, React.createElement("b", null, li[0]), li[1]))), s.install && React.createElement("div", {
