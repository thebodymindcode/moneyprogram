...
function Dashboard({
  ...
  return React.createElement("div", {
    ...
    className: "span2 only-mobile"
  }, React.createElement(InstallButton, null)), React.createElement("div", {
    className: "span2"
  }, React.createElement("div", {
    className: "head-row",
    style: {
...
const GUIDE_STEPS = [{
  ...
  list: [["Мой прогресс.", " Главная страница. Тут видно, сколько дней вы уже прошли и какой день у вас сегодня."], ["Карта дней.", " Все 17 дней по порядку. Видно, что пройдено, что сегодня, а что пока закрыто."], ["Дневник.", " Сюда сами собираются все ваши ответы и заметки за все дни. В одном месте."], ["Инструкция.", " Эта страница. Сюда можно вернуться в любой момент, если что-то забылось."]],
  shot: {
  ...
  body: ["Наверху страницы дня стоит проигрыватель. Нажмите круглую кнопку с треугольником, и аудио начнёт играть. Чтобы поставить на паузу, нажмите ту же кнопку ещё раз.", "Слушать удобнее в наушниках или просто включить погромче.", "Аудио не включилось с первого раза? Не переживайте. Обновите страницу и нажмите кнопку снова. Чтобы обновить, потяните страницу пальцем сверху вниз, либо нажмите кружок со стрелкой наверху браузера."],
  shot: {
  ...
  body: ["Ниже текста идут задания дня. Их немного. Под каждым заданием есть поле, куда можно вписать ответ.", "Впишите ответ своими словами и нажмите кнопку «Готово». Задание отметится зелёной галочкой.", "Ответ сохраняется сам. Отдельно сохранять ничего не надо. Захотите поправить ответ, нажмите «Изменить»."],
  shot: {
  ...
  body: ["В самом низу дня есть «Заметка дня». Это место для короткой мысли или вывода. Захотелось что-то записать, впишите.", "Заметка сохраняется сама, как только вы нажмёте в другое место. Потом все заметки и ответы соберутся в разделе «Дневник»."],
  shot: {
  ...
  body: ["Дни открываются по одному, по расписанию. Каждое утро открывается следующий день. Поэтому завтрашний день пока закрыт. На нём виден замочек и подпись, когда он откроется.", "Это сделано специально. Так у вас каждый день один спокойный шаг, без спешки.", "Завтра просто зайдите снова и откройте новый день. Пропустили день, ничего страшного. Он не пропадёт, вы спокойно вернётесь к нему позже."],
  shot: {
...
function Guide() {
  ...
  return React.createElement("div", {
    ...
    return React.createElement("div", {
      ...
      key: j
    }, React.createElement("b", null, li[0]), li[1])))), s.shot && React.createElement(GuideShot, {
      src: s.shot.src,
      cap: s.shot.cap
    }));
  }), React.createElement("div", {
